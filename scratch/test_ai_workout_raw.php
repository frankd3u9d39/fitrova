...
function callGeminiLocal($prompt, $apiKey, $primaryModel = 'gemini-1.5-pro-latest', $temp = 0.7) {
    $models = [$primaryModel, 'gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-3.5-flash', 'gemini-flash-latest'];
    ...
    foreach ($models as $modelName) {
        ...
        echo "Trying $modelName -> HTTP Code: $httpCode\n";
...
try {
    ...
    echo "\n--- Raw AI Response ---\n";
    echo substr($aiResponse, 0, 500) . "...\n";
    file_put_contents(__DIR__ . '/raw_response.txt', $aiResponse);
    ...
    if ($decoded === null) {
        echo "JSON Decode Error: " . json_last_error_msg() . " (Code: " . json_last_error() . ")\n";
    } else {
        echo "JSON Decoded Successfully!\n";
        echo "Has todays_workout: " . (isset($decoded['todays_workout']) ? "YES" : "NO") . "\n";
        if (isset($decoded['todays_workout'])) {
            echo "Workout Name: " . $decoded['todays_workout']['name'] . "\n";
            echo "Exercises Count: " . count($decoded['todays_workout']['exercises']) . "\n";
        }
    }
} catch (Exception $e) {
