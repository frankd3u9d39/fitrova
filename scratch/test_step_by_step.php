...
function callGeminiLocal($prompt, $apiKey, $modelName, $temp = 0.7) {
    ...
    echo "Gemini Call Status: $httpCode\n";
    if ($httpCode === 200) {
...
try {
    $aiResponse = callGeminiLocal($prompt, $GEMINI_API_KEY, $PRIMARY_MODEL, $AI_TEMPERATURE);
    echo "AI response fetched!\n";
    ...
    if (!$workoutData) {
        ...
        echo "Raw clean response snippet:\n" . substr($aiResponse, 0, 500) . "\n";
        exit();
    }
    
    echo "JSON Decoded Successfully!\n";
    echo "Has todays_workout: " . (isset($workoutData['todays_workout']) ? "YES" : "NO") . "\n";
    echo "Has todays_workout exercises: " . (isset($workoutData['todays_workout']['exercises']) ? "YES" : "NO") . "\n";
    
    if (isset($workoutData['todays_workout']['exercises'])) {
        echo "Exercises list before matching:\n";
        foreach ($workoutData['todays_workout']['exercises'] as $ex) {
            echo "- " . $ex['name'] . "\n";
        }
    }
    
    // Now let's try the matching part
    echo "\nStarting exercise video matching...\n";
    
    // Recreate library map
    ...
    require_once __DIR__ . '/../backend/app/controllers/workout/ai_workout_gemini.php';
    ...
    if (isset($workoutData['todays_workout']['exercises'])) {
        ...
        foreach ($workoutData['todays_workout']['exercises'] as $idx => &$exercise) {
            ...
            echo "Matching exercise: {$exerciseName} (search term: {$searchTerm})...\n";
            ...
            echo "Matched! Video: " . $media['video'] . " | Took " . round($duration, 2) . "s\n";
    ...
    echo "Is active fallback triggered? " . ((!$workoutData || isset($workoutData['error']) || !isset($workoutData['todays_workout'])) ? "YES" : "NO") . "\n";
